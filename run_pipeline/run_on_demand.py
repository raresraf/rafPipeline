...
def run_pipeline(solution_path, problemID):
	...
	for root, dirs, files in os.walk(f"../../TheInputsCodeforces/{problemID}"):
		for name in files:
			...
			output_path = solution_path.replace("TheCrawlCodeforces", "TheOutputsCodeforces") + f"/{input_size}.OUT"
			try:
				subprocess.call(["./run_atomic.sh",  solution_path, input_path, output_path], shell=False, timeout=TIMEOUT_RUN_ON_DEMAND)
			except Exception:
				pass
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
    	print("Usage: python3 run_on_demand <problemID>")
    	sys.exit(1)
    ...
    with open("../dataset_metadata/codeforce_libc.json") as f:
	    ...
	    for solution_path in metadata.get(problemID, None):
	    	# remove .CPP
		    run_pipeline(solution_path[:-4], problemID)
